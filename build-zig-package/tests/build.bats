...
setup() { ROOT="$BATS_TEST_TMPDIR/root"; BIN="$ROOT/bin"; mkdir -p "$BIN" "$ROOT/pkg" "$ROOT/target/release"; export PATH="$BIN:$PATH" GITHUB_WORKSPACE="$ROOT" CARGO_TARGET_DIR="$ROOT/target" RUNNER_OS=Linux; SCRIPT="$BATS_TEST_DIRNAME/../scripts/build.sh"; }
